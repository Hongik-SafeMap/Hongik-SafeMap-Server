...
public interface SafetyTipRepository extends JpaRepository<SafetyTip, Long> {
    ...
    @Query("SELECT st FROM SafetyTip st LEFT JOIN FETCH st.actions LEFT JOIN FETCH st.supplies LEFT JOIN FETCH st.warnings WHERE st.disasterType.id = :disasterTypeId")
    Optional<SafetyTip> findByDisasterTypeIdWithActions(@Param("disasterTypeId") Long disasterTypeId);
