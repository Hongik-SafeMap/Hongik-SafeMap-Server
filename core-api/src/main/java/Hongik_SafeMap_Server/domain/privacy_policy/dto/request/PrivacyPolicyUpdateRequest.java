...
public record PrivacyPolicyUpdateRequest(
        @Schema(description = "수정할 버전", example = "1.0")
        ...
        @Schema(description = "수정할 개인정보처리방침 항목 목록")
        @NotEmpty(message = "항목 목록은 비어있을 수 없습니다.")
        @Valid
        List<PrivacyPolicyUpdateItemRequest> sections
) {
    public record PrivacyPolicyUpdateItemRequest(
